...
Absence is one half of the problem. The other half is tests that do run and still prove nothing: assertions that pass trivially, collaborators mocked so hard the test only proves the mock works, edge cases nobody asked for. I wrote up [the five traps I check before trusting an AI-written test](/blog/ai-junit-tests/) separately. You need both questions, and this one comes first: did it run, and then, did it mean anything. The cleanest example I have of the second half is a [two-run security experiment where both suites went green and neither proved anything](/blog/codex-spring-boot-4-403-fix/): two passing tests could not tell a scoped fix from one that switched CSRF off application-wide, and only a probe outside the tested path separated them.
...
The same shape turns up outside the test runner. A rewritten Flyway migration is invisible to any suite that provisions a fresh database, because a database with no history has nothing to validate against: [CI stays green while the schema quietly diverges](/blog/flyway-checksum-mismatch-ai-agent/) from the database that actually ran the original migration. Both failure modes are "green build, wrong system", and both need you to ask a question the build is not asking.

The third version of it arrived from the other direction — not a test that never got discovered, but a test something deliberately switched off. An agent given a blocked Docker socket could not run my Testcontainers integration test, so it added `disabledWithoutDocker = true` and reported a green build. [Maven exits 0 with `Skipped: 1` and the test never executes](/blog/codex-testcontainers-docker-socket-spring-boot/), which is this post's whole thesis with the agent holding the lever.

A fourth version of it turned up when I asked the same question of Gradle instead of Maven, and the mechanism turned out to be a different one. The test class is discovered correctly, and the task would run; what gets served instead is a previous outcome, so `./gradlew test` prints `BUILD SUCCESSFUL` in 401ms while executing nothing. Maven hides files. Gradle replays results. I measured it on Spring Boot 4.1.1, including the fresh-checkout CI case where the build cache restores a green result for a suite that actually fails, in [a separate Gradle lab](/blog/gradle-build-successful-zero-tests-spring-boot-4/).
