...
One honest gap: I intended to run this same broken project through Codex as a third arm. The Codex CLI on this machine was pinned to an older version whose default model rejected the request, and once I worked around that, the workspace sandbox rejected every file operation with `sandbox_apply: Operation not permitted`, so it never reached the code. I said I would run it and report back rather than guess, so I did: **[the Codex run is here](/blog/codex-spring-boot-4-403-fix/)**. It reached the same two answers this project produced — a global `csrf.disable()` with nothing in the repo, a scoped `ignoringRequestMatchers("/api/**")` with the `AGENTS.md` present — and a probe I ran afterwards showed the difference that both green test suites had hidden. That piece also explains the sandbox refusal: a nested Seatbelt cannot initialise inside an already-sandboxed shell, which was my environment, not a Codex limitation. I have corrected the wording above accordingly, because the open question was mine to close.
