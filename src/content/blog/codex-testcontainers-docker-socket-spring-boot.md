...
If you are doing the broader Boot 4 move alongside this, the mechanical renames and the silent behaviour changes are catalogued in [the migration write-up](/blog/spring-boot-4-migration-ai-agents/), and this is one more entry in the same category: nothing fails loudly, the build goes green, and the evidence lives somewhere other than the build output. The same shape turns up one step earlier in the workflow: ask an agent to [add a Spring Boot 4 dependency](/blog/codex-spring-boot-4-dependencies-silent-failures/) and five of twelve will build green while wiring nothing. The [Java + AI hub](/blog/ai-for-java-developers/) collects the rest of these agent-on-JVM experiments.
