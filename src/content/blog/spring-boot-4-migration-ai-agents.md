...
faq:
  ...
  - question: "Why does my REST API return 403 after upgrading to Spring Boot 4?"
    answer: "Spring Security 7 ships with Boot 4 and enables CSRF protection for API endpoints by default, where it previously applied mainly to form-based flows. GET requests keep working while POST, PUT and DELETE return 403 with nothing useful in the logs. If GET passes and POST fails with the same token, that is CSRF. Confirm it with logging.level.org.springframework.security.web.csrf set to DEBUG. For a stateless JWT API, scope the exemption to your API matcher rather than disabling CSRF globally."
...
**Spring Security 7 enables CSRF for API endpoints by default.** This one is the standout. Your upgrade compiles, your tests pass, and then production returns 403 on every write. GET keeps working, POST/PUT/DELETE all fail, and there is nothing in the logs to explain it. The tell is simple: if GET succeeds and POST fails with the same credentials, it is CSRF. Confirm it with:
